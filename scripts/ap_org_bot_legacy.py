...
# ── Phase A: Notion writer (opt-in via NOTION_API_KEY in .env.antique) ──
try:
    import sys as _sys_for_notion
    _sys_for_notion.path.insert(0, str(Path(__file__).parent))
    ...
    _lg.getLogger("ap_org_bot").warning("[notion] notion_writer not loaded: %s", _e)
    def notion_enabled(): return False
    def create_topic(*a, **k): return None
    def create_decision(*a, **k): return None
    def create_content_calendar(*a, **k): return None
    def create_incident(*a, **k): return None
...
async def generate_proposals(feedback_messages: list[dict]) -> list[dict]:
    ...
    try:
        ...
        if isinstance(proposals, list):
            # ── Phase A: 寫每筆提案到 Topics Notion DB (opt-in) ──
            if notion_enabled():
                for p in proposals:
                    try:
                        # AP feedback 類型 → Topics 類型對應
                        category_map = {
                            "視覺設計": "視覺",
                            "互動體驗": "視覺",
                            "效能":     "維運",
                        ...
                        cat = category_map.get(p.get("category", ""), "其他")
                        ...
                        log.warning("[notion] topic write failed: %s", _e)
            return proposals
        return []
...
async def _spawn_marketing_headless(topic: str, ticket_id: str, original_message: discord.Message):
    marketing_prompt = (
        ...
        "**特別注意「Voice Layers」段：Layer 1（對外顧客文字）vs Layer 2（對內策略建議）語氣完全不同**。\n"
    ...
    # ── Phase A: 寫到 Content Calendar Notion DB (opt-in) ──
    if notion_enabled():
        try:
            create_content_calendar(
                title=f"[{ticket_id}] {topic[:60]}",
                content_type="社群貼文",
                status="撰寫中",
                owner="Marketing",
                keywords=topic[:200],
                notes=agent_text[:1500],
            )
        except Exception as _e:
            log.warning("[notion] content calendar write failed: %s", _e)
...
async def _spawn_auto_execute_task(proposal: dict, poll_id: str, channel: discord.TextChannel):
    ...
    # ── Phase A: 寫到 Decisions Notion DB (opt-in) ──
    if notion_enabled():
        try:
            success = "✅" in result or "自動執行完成" in result
            create_decision(
                title=f"[{ticket}] {title}",
                signoff_status="通過" if success else "重議",
                tldr=title[:300],
                recommended=solution[:1500],
                risks=problem[:1500],
                follow_up=result[:1500],
            )
        except Exception as _e:
            log.warning("[notion] decision write failed: %s", _e)
